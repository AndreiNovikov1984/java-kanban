...
public class Sort {
    static Comparator<Task> taskComparator = new Comparator<>() {
        ...
        public int compare(Task task1, Task task2) {
            if (task1.getTaskStartTime() == null) {return 5;}
            if (task2.getTaskStartTime() == null) {return -5;}
            if (task1.getTaskStartTime().isAfter(task2.getTaskStartTime())) {return 5;
            } else { return -5;}
        }
    };
    ...
    protected static void sort(Map<Integer, Task> listTask, Map<Integer, SubTask> listSubTask){
        ...
        for (Task task : listTask.values()) { sortedTaskTree.add(task);}
        for (SubTask task : listSubTask.values()) { sortedTaskTree.add(task);}
    }
